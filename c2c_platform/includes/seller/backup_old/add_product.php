    ...
    if (!isset($_POST['csrf_token'])) {
        ...
        try {
            ...
            foreach ($_POST['variation_name'] as $index => $name) {
                if (empty(trim($name))) {
                    error_log("Skipping empty variation at index $index");
                    continue;
                }
                ...
                error_log("Created variation ID $variationId for index $index");

                // Handle file uploads
                if (!empty($_FILES['variation_images']['name'][$index])) {
                    error_log("Processing files for variation index $index");

                    // Get all files for this variation
                    $fileData = [
                        'name' => (array) $_FILES['variation_images']['name'][$index],
                        'tmp_name' => (array) $_FILES['variation_images']['tmp_name'][$index],
                        'error' => (array) $_FILES['variation_images']['error'][$index]
                    ];
                    ...
                    foreach ($fileData['name'] as $fileIndex => $fileName) {
                        if ($fileData['error'][$fileIndex] === UPLOAD_ERR_OK && !empty($fileName)) {
                            $validFiles[] = $fileName;
                            $validTmpFiles[] = $fileData['tmp_name'][$fileIndex];
                            error_log("Added file: $fileName");
                        }
                    }

                    if (!empty($validFiles)) {
                        error_log("Uploading " . count($validFiles) . " files for variation $variationId");
                        $product->uploadVariationImages($variationId, $validFiles, $validTmpFiles);
                    }
                } else {
                    error_log("No files submitted for variation index $index");
                }
            }
                                    ...
                                    <div class="card-body">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <input type="file" class="form-control" name="variation_images[]"
